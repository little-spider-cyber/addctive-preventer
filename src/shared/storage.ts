...
class StorageManager {
  private async get<K extends keyof StorageSchema>(key: K): Promise<StorageSchema[K]> {
    const result = await chrome.storage.local.get(key);
    return result[key];
  }

  private async set<K extends keyof StorageSchema>(key: K, value: StorageSchema[K]): Promise<void> {
  ...
  private ensureDateObjects(prompt: any): CustomPrompt {
    return {
      ...prompt,
      createdAt: prompt.createdAt instanceof Date ? prompt.createdAt : new Date(prompt.createdAt || Date.now()),
      lastUsed: prompt.lastUsed instanceof Date ? prompt.lastUsed : new Date(prompt.lastUsed || 0)
    };
  }
  ...
  async addCustomPrompt(prompt: Omit<CustomPrompt, 'id' | 'createdAt' | 'lastUsed' | 'effectiveness'>): Promise<void> {
  ...
  async updatePromptEffectiveness(promptId: string, wasEffective: boolean): Promise<void> {
    const prompts = await this.getCustomPrompts();
  ...
  async getReflectionEntries(): Promise<ReflectionEntry[]> {
    ...
    // Convert date strings back to Date objects safely
    return defaultEntries.map(entry => this.ensureReflectionDateObjects(entry));
  }

  async addReflectionEntry(entry: Omit<ReflectionEntry, 'id' | 'timestamp'>): Promise<void> {
    ...
    retentionDate.setDate(retentionDate.getDate() - settings.privacy.dataRetentionDays);
    
    const filteredEntries = settings.privacy.saveReflections 
      ? updatedEntries.filter(e => new Date(e.timestamp) > retentionDate)
      : [];
    ...
    const wasEffective = entry.action === 'redirected' || entry.action === 'took-break';
    const prompts = await this.getCustomPrompts();
  ...
  private async updateSuccessMetrics(entry: ReflectionEntry): Promise<void> {
    ...
    const updatedMetrics: SuccessMetrics = {
      mindfulSessions: wasEffective ? metrics.mindfulSessions + 1 : metrics.mindfulSessions,
      ...
      interventionEffectiveness: (metrics.interventionEffectiveness * metrics.reflectionEntries + (wasEffective ? 1 : 0)) / (metrics.reflectionEntries + 1),
      averageReflectionTime: (metrics.averageReflectionTime * metrics.reflectionEntries + entry.responseTime) / (metrics.reflectionEntries + 1),
  ...
  async isBlacklisted(domain: string): Promise<boolean> {
    ...
    return blacklist.some(blocked => {
      ...
      // Subdomain matching
      if (domain.endsWith('.' + blocked) || blocked.endsWith('.' + domain)) return true;
      if (domain.endsWith('.' + blockedWithoutWww) || domainWithoutWww.endsWith('.' + blocked)) return true;
      
      // Wildcard matching
      if (blocked.startsWith('*.') && domain.endsWith(blocked.slice(2))) return true;
  ...
  async selectPrompt(): Promise<CustomPrompt> {
    ...
    switch (settings.promptConfig.rotationType) {
      ...
      case 'weighted':
        ...
        const totalWeight = prompts.reduce((sum, p) => sum + (p.effectiveness + 0.1), 0);
